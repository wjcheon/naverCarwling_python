...
for iterKeyword in diseaseList:
    ...
    if not pd.isna(iterKeyword):
        ...
        while True:
            ...
            # Categorical address + allDate
            # https://kin.naver.com/search/list.naver?query=%EC%86%8C%EC%95%84%EC%B2%AD%EC%86%8C%EB%85%84%EA%B3%BC&section=qna&dirId=70115&page=1
            driver.get('https://kin.naver.com/search/list.nhn?' + '&query=' + get_keyword(
                keyword) + "&section=qna&dirId=70115" + "&page=" + str(page_index))
        ...
        count=0
        page_url_unique = list(set(page_url))
        print("Original size: {}, Unique size: {}".format(len(page_url), len(page_url_unique)))
        for i in page_url_unique:
            ...
            try:
                ...
                question_txt = ""
                print("try and except in question_txt")
                continue
